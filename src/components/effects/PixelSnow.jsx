/**
 * [INPUT]: 依赖 three 的 WebGLRenderer/ShaderMaterial/Scene 等，依赖 react 的 hooks
 * [OUTPUT]: 导出 PixelSnow 组件 —— 全屏 raymarching 体积雪花着色器背景
 * [POS]: effects 模块唯一成员，被 landing/Hero 作为装饰背景层消费；颜色由设计系统 token 运行时注入
...
// ════════════════════════════════════════════════════════════════
//  Shaders —— 顶点直通 + 片元体积步进（raymarching）雪花场
// ════════════════════════════════════════════════════════════════
const vertexShader = `
void main() {
...
// ════════════════════════════════════════════════════════════════
//  CSS Token Resolver —— 让浏览器充当解析器
//  把任意 CSS 颜色（含 oklch / var(--token)）解析为线性 sRGB 三元组
//  好品味：零转换库、零硬编码，颜色永远活在设计系统里
// ════════════════════════════════════════════════════════════════
function resolveTokenColor(cssColor) {
  ...
  useEffect(() => {
    ...
    const material = new ShaderMaterial({
      ...
      uniforms: {
        ...
        uColor: { value: resolveTokenColor(colorToken) },
    ...
    // 主题切换时重新解析 token 颜色（next-themes 切换 <html> class）
    const themeObserver = new MutationObserver(() => {
      material.uniforms.uColor.value.copy(resolveTokenColor(colorToken));
    });
